...
import { comicDefaultValues } from '@/lib-bak/constants';
import { insertComicSchema } from '@/lib-bak/validators';

import { createComic } from '@/lib-bak/actions/comic.actions';
...
const ComicCreateForm = () => {
  ...
  const form = useForm<z.infer<typeof insertComicSchema>>({
    resolver: zodResolver(insertComicSchema),
    defaultValues: comicDefaultValues,
  });
  const onSubmit: SubmitHandler<z.infer<typeof insertComicSchema>> = async (
    values
  ) => {
    const res = await createComic(values);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <FormField
            ...
            render={({
              ...
              field: ControllerRenderProps<
                z.infer<typeof insertComicSchema>,
                'title'
              >;
            }) => (
        ...
        <div>
          ...
          >
            {form.formState.isSubmitting ? 'Submitting' : `Create Comic`}
          </Button>
        </div>
      </form>
    </Form>
