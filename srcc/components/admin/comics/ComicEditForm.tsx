...
import {
  ...
  FormLabel,
  FormMessage,
} from '@/srcc/components/ui/form';
import { Input } from '@/srcc/components/ui/input';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/srcc/components/ui/select';
import { useToast } from '@/srcc/hooks/use-toast';
import { comicDefaultValues } from '@/srcc/lib/constants';
import { updateComicSchema } from '@/srcc/lib/validators';
...
interface ComicEditFormProps {
  item: Comic | undefined;
}

const ComicEditForm = ({ item }: ComicEditFormProps) => {
  const { toast } = useToast();

  const form = useForm<z.infer<typeof updateComicSchema>>({
    resolver: zodResolver(updateComicSchema),
    defaultValues: item ? item : comicDefaultValues,
  });
